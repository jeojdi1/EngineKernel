...
class Qwen3(torch.nn.Module):
    def __init__(self, model_path: str, device: torch.device, dtype=torch.bfloat16):
        ...
        # fenced (see _rope_attn_verify_kernel); +1% at batch 1, its remaining
        # differences from the unfused path are 1-ulp bf16 reassociation
    ...
    def prefill(self, input_ids, positions, k_cache, v_cache, attn_bias=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if (self._sdpa_cudnn is None and self.prefill_triton and attn_bias is None
                    and self._sdpa_gqa):
                # the sandbox cannot run cuDNN's attention; this beats torch's flash
